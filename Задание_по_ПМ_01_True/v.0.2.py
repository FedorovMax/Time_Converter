...
#Размеры окна
main_root.geometry("300x300+100+100") #Ширина на высоту + отступы от верхнего левого угла на 100 px
...
label_main_root_image = Label(main_root, image = image_main_root) #Установка картинки
...
def open_txt():
	...
	if button_open_txt['text'] == 'Показать Историю':
		...
		main_root.geometry("600x300") #Ширина на высоту
		...
		main_root.geometry("300x300") #Ширина на высоту
...
def author_info_on(event):
	#Создаем label
	global label_bg_info, label_info #Делаем глобальными, для избежания проблем
...
#Функция - 'Поле с информацией'
global showinfo_on #Делаем глобальным, для избежания проблем
def showinfo_on():

	def showinfo_off():
		label_bg_showinfo_.place_forget() #Скрываем виджет
		label_showinfo_.place_forget() #Скрываем виджет
		button_ok.place_forget() #Скрываем виджет
		label_bg_ok.place_forget() #Скрываем виджет

	label_bg_showinfo_ = Label(main_root, bg = '#77BDE4')
	label_bg_showinfo_.place(x = 5, y = 5, height = 245, width = 290)

	label_showinfo_ = Label(main_root, bg = '#20329B', 
		text = 'Информация\n***\n1 - Выберите тип конвертации\n***\n2 - Можно вводить\nтолько числа'+
		'\n***\n3 - Отрицательные числа\nвводить нельзя\n***\nНапример: 2, 5, 0.5, и т.п.', 
		fg = '#77BDE4', font = '12')
	label_showinfo_.place(x = 7, y = 7, height = 241, width = 286)

	#Кнопочка - 'Ок'//'Понятно'
	label_bg_ok = Label(main_root, bg = '#77BDE4')
	label_bg_ok.place(x = 5, y = 255, height = 40, width = 290)
	button_ok = Button(main_root, bg = '#20329B', text = 'Понятно', fg = '#77BDE4',
		font = '12', bd = '0', activebackground = '#333FEB', command = showinfo_off)
	button_ok.place(x = 7, y = 257, height = 36, width = 286)
	#
	button_ok.bind("<Enter>", on_button_color)
	button_ok.bind("<Leave>", off_button_color)
	...
	if len_array == 1: #Дни в минуты
		if array_type[0] == 0:
			try:
				...
				if days < 0: #Нам не нужны отрицательные значения
					showinfo_on() #Вызываем подсказку!
				else:
					minutes = days * 1440 #Ну в дне 1440 минут
					entry_f.delete(0, END)
					#Вывод
					entry_f.insert(0, round(minutes, 5)) #rоund - округление
					#Время
					time = strftime("%H:%M:%S") #Часы, Минуты, Секунды //Чтобы запись в историю выглядела более информативной
					#Запись в историю
			...
			try:
				...
				if minutes < 0: #Нам не нужны отрицательные значения
					showinfo_on()
				else: 
					days = minutes / 1440 #Ну в дне 1440 минут
					entry_f.delete(0, END)
					#Вывод
...
global on_button_color, off_button_color
def on_button_color(event):
	event.widget['bg'] = '#233BC8' #Новый цвет фона кнопки
	event.widget['fg'] = '#77BDE4' #Новый(Нет) цвет текста конопки
def off_button_color(event):
	event.widget['bg'] = '#20329B' #Стандартный цвет фона кнопки
	event.widget['fg'] = '#77BDE4' #Стандартный цвет текста кнопки
...
button_start.place(x = 7, y = 212, height = 36, width = 286)
#
button_start.bind("<Enter>", on_button_color) #Курсор находится на кнопке //И так со всеми кнопками
button_start.bind("<Leave>", off_button_color) #Курсор не на кнопке //И так со всеми кнопками
